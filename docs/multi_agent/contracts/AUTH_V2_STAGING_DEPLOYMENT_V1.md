...
## 13. Post-execution incident disposition — 2026-08-27

This section records a one-time governance incident discovered during closure of `AUTH-V2-BE-STAGING-DEPLOY-001`. It is an Architecture-owner disposition of an already-completed execution sequence; it does **not** retroactively redefine §8 and does not weaken the mandatory pre-mutation gate for any future deployment.

### 13.1 Incident

Required sequence under §8:

```text
pre-mutation staging migrate status/verify
→ apply reviewed migrations
→ replace staging-api
→ post-deploy verification
```

Actual RC3 execution sequence:

```text
fresh pre-release staging audit showed 47 successful migrations
+ disposable RC2→049 rehearsal/compatibility PASS
→ RC3 startup migration runner applied 048/049
→ staging-api RC3 started
→ exact RC3 cmd/migrate status: pending=0
→ exact RC3 cmd/migrate verify: all applied checksums match
```

The staging `cmd/migrate status/verify` CLI was not rerun immediately before RC3 startup. That omission is a real noncompliance with §8 step 8. Successful post-deploy verification is evidence of the resulting state; it is not treated as proof that the missing pre-mutation gate occurred.

### 13.2 Risk and runtime disposition

Architecture authority classifies this as an **execution-order governance incident**, not evidence of schema corruption or an authorization expansion. The following independently reproducible post-state evidence exists:

- migration ledger contains 49 successful rows and zero failed rows;
- exact RC3 migration tooling reports no pending migration and verifies all applied checksums;
- migration 048/049 expected schema constraints are present;
- RC3 health and shared-consumer compatibility smokes pass;
- Auth V2 fails closed with provider/email configuration absent;
- post-deploy inventory/drift is clean and only the canonical `staging` Compose project exists;
- RC2 service-image compatibility against post-049 schema was proven separately; schema rollback is not claimed.

Given that the forward migrations are already applied and mechanically verified, Architecture authority does **not** authorize destructive rollback, replay, direct SQL repair, artificial re-deployment solely to manufacture a pre-mutation timestamp, or provider/config mutation. The existing RC3 staging runtime may remain in service while this incident disposition is reviewed.

### 13.3 Closure rule for this incident

`AUTH-V2-BE-STAGING-DEPLOY-001` may close only after all of the following are true:

1. this incident disposition is independently reviewed on an exact Nebula governance candidate and becomes canonical;
2. FlyPostAPI closure evidence cites the canonical disposition and continues to state that the pre-start CLI check was omitted;
3. the closure evidence must not use post-deploy verification as a substitute for the missing pre-mutation event;
4. all existing RC3 runtime, migration, compatibility, drift, provenance and lease-release evidence remains valid.

If independent Architecture review rejects this disposition, the execution Story remains open and the reviewer determines the required governance corrective; no Backend production/runtime mutation is implied by default.

### 13.4 Preventive control

For every future deployment governed by this contract, §8 step 8 remains fail-closed. The deployment operator must capture a timestamped `migrate status` and `migrate verify` result from the exact reviewed migration tooling **after the deploy lease is acquired and before any migration runner or application container capable of mutation is started**. Absence of that evidence is a STOP condition.

This incident creates no reusable waiver, no exception class, and no precedent for replacing pre-mutation gates with post-mutation checks.
