...
| F1-03..F6 | BLOCKED | - | Follow dependencies in implementation plan |
...
`F1-03` in nebula-flutter-sdk: Secure/Cache Storage ports and namespace (docs/03 F1, storage, depends on F0). **F1-02 is complete**: `NebulaSessionAuth` implements `NebulaAuth` and performs real login/refresh/logout HTTP calls over `HttpTransport` with proof headers, single-flight refresh (401 storm → one network call), typed error mapping and `restoreSession`; all gates green (dart test 82, dart analyze, governance/api_surface/secret_scan PASS; api_surface 75 symbols; ArchGuard/Sentinel not applicable to SDK). F1-01 and F1-02 closure verified.
