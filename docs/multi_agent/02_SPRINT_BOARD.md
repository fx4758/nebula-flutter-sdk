...
状态：**IN PROGRESS / GOVERNED**。S1-F01-001 已通过 R2 独立验收并 DONE；S1-F01-002 维持 WAIT：NFC Writer 的 NEBULA-DEP-001/001A 已完成，但 preflight 发现 bootstrap frozen-contract/public-surface drift；须先 S1-F01-003 contract reconciliation → S1-F01-004 SDK surface closure → App NEBULA-DEP-002 immutable repin；其余 Story 受依赖和单仓执行规则控制。
...
| S1-F01-002 NFC Writer Reference Bootstrap Integration | Agent A | **WAIT / SDK BOOTSTRAP + APP REPIN GATE** | Flutter NFC Writer | `s1/f01-002-bootstrap` |
| S1-F01-003 Bootstrap Contract Reconciliation | Agent D | **READY** | nebula-flutter-sdk | `s1/f01-003-bootstrap-contract-reconcile` |
| S1-F01-004 SDK Bootstrap Surface Closure | Agent D | **WAIT (after F01-003)** | nebula-flutter-sdk | `s1/f01-004-sdk-bootstrap-surface` |
