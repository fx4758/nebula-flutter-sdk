...
| F2-01 | BLOCKED | Codex | Effective config/feature/version API contract — blocked by missing flypost mobile config contract (reproducible, verified 2026-08-04): `flypost/sdk/CONTRACT.md:165` marks 产品/配置/Feature 下发 as **待办**; the only registered config/feature routes are admin/product-control-plane (`/products/:id/effective-configs`, `/products/:id/effective-features`, `/admin/config`, `/ai/config`) which ADR-F005 keeps out of the mobile SDK; no `/api/v1/mobile/config*` or version route exists (installation-token scope comment mentions config/version as *intended* entries only). docs/01 §6 requires route exists + CONTRACT.md registration + integration test before any SDK capability — none hold. Inventing the wire shape is forbidden by docs/00 禁止事项. Unblock: freeze a flypost mobile config/version contract first (needs explicit authorization) |
...
`F2-01` in nebula-flutter-sdk: effective config/feature/version API contract — **BLOCKED (verified 2026-08-04)**: the flypost mobile config contract is not frozen (`flypost/sdk/CONTRACT.md:165` 待办; no `/api/v1/mobile/config*` route; all existing config routes are admin/product-control-plane per ADR-F005). docs/01 §6 + docs/00 禁止事项 forbid inventing the wire shape, so no SDK code can be claimed. **F1 is complete**: F1-01..F1-05 all DONE — kernel network layer, user session capability, storage ports + namespace, redacted logging/error classification/request ID, and the fake-transport kernel integration test. F1 exit criteria met: all error paths verifiable via `FakeTransport` without a real backend; concurrent refresh triggers exactly one network call. Gates green (dart test 123, dart analyze, governance/secret_scan PASS; api_surface 91 symbols).
