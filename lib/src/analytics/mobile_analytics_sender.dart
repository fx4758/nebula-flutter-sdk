...
final class MobileAnalyticsSender
  ...
  AssignedMobileAnalyticsBatch assignBatch(
    ...
    if (utf8.encode(batchId).isEmpty || utf8.encode(batchId).length > 128) {
    ...
    while (count > 0) {
      final List<NebulaAnalyticsEvent> events =
          List<NebulaAnalyticsEvent>.unmodifiable(
        candidates.take(count),
      );
  ...
  Future<MobileAnalyticsSendDisposition> sendAssigned(
    ...
    _cooldownUntil = null;
    if (batch.events.isEmpty || batch.events.length > _maxAnalyticsEvents) {
      return MobileAnalyticsSendDisposition.nonRetryable;
    }
    if (batch.encodedBytes > _maxAnalyticsRequestBytes ||
