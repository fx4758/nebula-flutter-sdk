...
| F1-05..F6 | BLOCKED | - | Follow dependencies in implementation plan |
...
`F1-05` in nebula-flutter-sdk: fake transport and kernel integration test (docs/03 F1, testing, depends on F1-01..04). **F1-04 is complete**: `NebulaRequestId` (client `X-Request-Id` + server echo preference), `NebulaErrorCategory`/`classifyNebulaError`, redacted `NebulaLogger` Port with `NoOpLogger`/`RedactingLogger`/`redact` (privacy-by-default: request id/endpoint/result/duration only), and transport-level log events; ADR-F010 raised `max_public_exports` to 40 for the frozen F1-F4 roadmap; all gates green (dart test 110, dart analyze, governance/secret_scan PASS; api_surface 89 symbols). F1-01..F1-04 closure verified.
